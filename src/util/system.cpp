...
const char * const PEPPACOIN_CONF_FILENAME = "peppacoin.conf";
const char * const PEPPACOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //peppacoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", PEPPACOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
#else
    const char* pszModule = "peppacoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Peppacoin
    // macOS: ~/Library/Application Support/Peppacoin
    // Unix-like: ~/.peppacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Peppacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Peppacoin";
#else
    // Unix-like
    return pathRet / ".peppacoin";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", PEPPACOIN_CONF_FILENAME));
}
