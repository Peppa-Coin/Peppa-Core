...
#include <config/peppacoin-config.h>
#endif

#include <qt/peppacoin.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<PeppacoinUnit>("PeppacoinUnit");
#else
    qRegisterMetaType<PeppacoinUnit>("PeppacoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. peppacoin_de.qm (shortcut "de" needs to be defined in peppacoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. peppacoin_de_DE.qm (shortcut "de_DE" needs to be defined in peppacoin.qrc)
...
static const char* qt_argv = "peppacoin-qt";

PeppacoinApplication::PeppacoinApplication()
...
void PeppacoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the PeppacoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", PeppacoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void PeppacoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool PeppacoinApplication::createOptionsModel(bool resetSettings)
{
...
void PeppacoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new PeppacoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &PeppacoinGUI::quitRequested, this, &PeppacoinApplication::requestShutdown);
...
void PeppacoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void PeppacoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool PeppacoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void PeppacoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &PeppacoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &PeppacoinApplication::handleRunawayException);
    connect(this, &PeppacoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &PeppacoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void PeppacoinApplication::parameterSetup()
{
...
void PeppacoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void PeppacoinApplication::requestInitialize()
{
...
void PeppacoinApplication::requestShutdown()
{
...
void PeppacoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // peppacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &PeppacoinGUI::handlePaymentRequest);
            connect(window, &PeppacoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void PeppacoinApplication::handleRunawayException(const QString &message)
{
...
void PeppacoinApplication::handleNonFatalException(const QString& message)
{
...
WId PeppacoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool PeppacoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", PeppacoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(peppacoin);
    Q_INIT_RESOURCE(peppacoin_locale);
    ...
    /// 6-7. Parse peppacoin.conf, determine network, switch to network specific
    ...
    // peppacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
