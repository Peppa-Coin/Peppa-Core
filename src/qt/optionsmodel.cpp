...
/** Call node.updateRwSetting() with Peppacoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Peppacoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/peppacoin/peppacoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because peppacoin 22.x will not try to read these.
...
//! Convert enabled/size values to peppacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from peppacoin -prune setting.
...
//! Get pruning size value to show in GUI from peppacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayPeppacoinUnit")) {
        settings.setValue("DisplayPeppacoinUnit", QVariant::fromValue(PeppacoinUnit::BTC));
    }
    QVariant unit = settings.value("DisplayPeppacoinUnit");
    if (unit.canConvert<PeppacoinUnit>()) {
        m_display_peppacoin_unit = unit.value<PeppacoinUnit>();
    } else {
        m_display_peppacoin_unit = PeppacoinUnit::BTC;
        settings.setValue("DisplayPeppacoinUnit", QVariant::fromValue(m_display_peppacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if peppacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_peppacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<PeppacoinUnit>() == m_display_peppacoin_unit) return;
    m_display_peppacoin_unit = new_unit.value<PeppacoinUnit>();
    QSettings settings;
    settings.setValue("DisplayPeppacoinUnit", QVariant::fromValue(m_display_peppacoin_unit));
    Q_EMIT displayUnitChanged(m_display_peppacoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/peppacoin/peppacoin/pull/8273
    ...
    // (https://github.com/peppacoin-core/gui/issues/567).
