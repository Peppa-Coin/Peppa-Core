...
#include <qt/peppacoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Peppacoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new PeppacoinAddressEntryValidator(parent));
    widget->setCheckValidator(new PeppacoinAddressCheckValidator(parent));
}
...
{
    // return if URI is not valid or is no peppacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("peppacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!PeppacoinUnits::parse(PeppacoinUnit::BTC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsePeppacoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parsePeppacoinURI(uriInstance, out);
}
...
QString formatPeppacoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("peppacoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PeppacoinUnits::format(PeppacoinUnit::BTC, info.amount, false, PeppacoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openPeppacoinConf()
{
    ...
    /* Open peppacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Peppacoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Peppacoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Peppacoin (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Peppacoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "peppacoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("peppacoin-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a peppacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Peppacoin\n";
        else
            optionFile << strprintf("Name=Peppacoin (%s)\n", chain);
