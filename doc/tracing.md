# User-space, Statically Defined Tracing (USDT) for Peppacoin Core

Peppacoin Core includes statically defined tracepoints to allow for more
...
**Note**: Peppacoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Peppacoin Core
...
Multiple tools can list the available tracepoints in a `peppacoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Peppacoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/peppacoind
stap net        outbound_message 0x0000000000107c05 /src/peppacoind
stap validation block_connected  0x00000000002fb10c /src/peppacoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Peppacoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/peppacoind | grep NT_STAPSDT -A 4 -B 2
