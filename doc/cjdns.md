# CJDNS support in Peppacoin Core

It is possible to run Peppacoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Peppacoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Peppacoin network and individual nodes.
...
before it will work with your Peppacoin Core node. This is described in steps
...
## Run Peppacoin Core with CJDNS

Once you are connected to the CJDNS network, the following Peppacoin Core
...
When enabled, this option tells Peppacoin Core that it is running in an
...
IPv6 local network. This helps Peppacoin Core perform better address management:
...
CJDNS support was added to Peppacoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `peppacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/peppacoin/peppacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Peppacoin Core

There are several ways to see your CJDNS address in Peppacoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `peppacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `peppacoin-cli getpeerinfo`).
