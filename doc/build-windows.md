...
Below are some notes on how to build Peppacoin Core for Windows.

The options known to work for building Peppacoin Core on Windows are:
...
Note that for WSL the Peppacoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/peppacoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\peppacoin`, for example:

    make install DESTDIR=/mnt/c/workspace/peppacoin
