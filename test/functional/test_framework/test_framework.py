...
TMPDIR_PREFIX = "peppacoin_func_test_"
...
class PeppacoinTestMetaClass(type):
    """Metaclass for PeppacoinTestFramework.

    Ensures that any attempt to register a subclass of `PeppacoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'PeppacoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("PeppacoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("PeppacoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class PeppacoinTestFramework(metaclass=PeppacoinTestMetaClass):
    """Base class for a peppacoin test script.

    Individual peppacoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave peppacoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop peppacoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use peppacoin-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_peppacoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "peppacoind" + config["environment"]["EXEEXT"],
        )
        fname_peppacoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "peppacoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_peppacoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "peppacoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.peppacoind = os.getenv("PEPPACOIND", default=fname_peppacoind)
        self.options.peppacoincli = os.getenv("PEPPACOINCLI", default=fname_peppacoincli)
        self.options.peppacoinutil = os.getenv("PEPPACOINUTIL", default=fname_peppacoinutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: peppacoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("PeppacoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'peppacoind', self.options.peppacoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'peppacoin-cli', self.options.peppacoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a peppacoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple peppacoind test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as peppacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    peppacoind=self.options.peppacoind,
                    peppacoin_cli=self.options.peppacoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in peppacoin.conf
    ...
    def skip_if_no_peppacoind_tracepoints(self):
        """Skip the running test if peppacoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("peppacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_peppacoind_zmq(self):
        """Skip the running test if peppacoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("peppacoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if peppacoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("peppacoin-wallet has not been compiled")

    def skip_if_no_peppacoin_util(self):
        """Skip the running test if peppacoin-util has not been compiled."""
        if not self.is_peppacoin_util_compiled():
            raise SkipTest("peppacoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if peppacoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("peppacoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether peppacoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether peppacoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_peppacoin_util_compiled(self):
        """Checks whether peppacoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_PEPPACOIN_UTIL")
