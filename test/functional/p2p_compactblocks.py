...
# TestP2PConn: A peer we use to send messages to peppacoind, and store responses.
...
class CompactBlocksTest(PeppacoinTestFramework):
    ...
    # This test actually causes peppacoind to (reasonably!) disconnect us, so do this last.
    ...
    # Test that peppacoind requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for peppacoind to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # peppacoind will not send blocktxn responses for blocks whose height is
