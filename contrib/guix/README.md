# Bootstrappable Peppacoin Core Builds

This directory contains the files necessary to perform bootstrappable Peppacoin
Core builds.
...
To build Peppacoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Peppacoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/peppacoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/peppacoin-core/guix.sigs).
