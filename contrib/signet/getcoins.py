#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Peppacoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing peppacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='peppacoin-cli', help='peppacoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Peppacoin address to which the faucet should send')
...
parser.add_argument('peppacoin_cli_args', nargs='*', help='Arguments to pass on to peppacoin-cli (default: -signet)')

args = parser.parse_args()

if args.peppacoin_cli_args == []:
    args.peppacoin_cli_args = ['-signet']


def peppacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.peppacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = peppacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = peppacoin_cli(['getnewaddress', 'faucet', 'bech32'])
